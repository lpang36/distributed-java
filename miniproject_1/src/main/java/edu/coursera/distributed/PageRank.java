...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer,Tuple2<Website,Double>> joint = sites.join(ranks);
        JavaPairRDD<Integer,Double> ratio = joint.mapValues(x -> x._2/x._1.getNEdges());
        JavaPairRDD<Integer,Integer> links = sites.flatMapValues(x -> () -> x.edgeIterator());
        JavaPairRDD<Integer,Tuple2<Integer,Double>> linkRatios = links.join(ratio);
        JavaPairRDD<Integer,Double> swapped = linkRatios.values().mapToPair((x) -> x);
        swapped = swapped.reduceByKey((x,y) -> x+y);
        return swapped.mapValues((x) -> 0.15+0.85*x);
    }
}
